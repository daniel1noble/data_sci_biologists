...
\end{equation} 
$$
From your meta-analytic model you get an estimate for all these variances, but we can use the sampling variance for each Zr effect size to calculate $\sigma^2_{m}$. To do this, we need to convert these to 'weights' which is simply $w_{i} = \frac{1}{v_{Zr}}$. $k$ in thsi equation is simply the total number of effects in the data set.
