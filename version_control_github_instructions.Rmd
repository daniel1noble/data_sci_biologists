...
3) add a new Rmarkdown file in this directory that has relevant yaml header with your name, date ect. Remember how we have been setting up our RMarkdown file structure. Make sure that you mimic that structure so that you can render the html version of it.
...
Open up your new Rmarkdown file that you created and are now tracking in your repo. Complete the following:

1) Edit this file by adding code to load the `OA_activitydat_20190302_BIOL3207.csv`. The `OA_activitydat_20190302_BIOL3207.csv` is a data set on ocean acidification effects on fish behaviour that was published in Nature in 2020. The detailed meta-data for each column are as follows:
...
2) Clean up the data. There are a bunch of extra columns and missing data in the dataset. Use your new found data wrangling skills to explicitly code a chunk to:
    a) Remove the missing data
    b) Check that there are no spelling issues in variables 
    c) Drop out columns that are not relevant (e.g, `comment` column and `loc` can probably go for now).
...
1) In RStudio, create a text file called `.gitignore` and save this file to the root of your repo. The "root" is the main directory folder that you cloned down. 2) Add `*.html` to your `.gitignore` file. Save this file. Now, knit your Rmarkdown document. What do you notice about your *GitHub Desktop* application when this file is created?
...
4) In your Rmarkdown file, use the data that you stored (i.e., `OA_activitydat_20190302_BIOL3207.csv`) to create a single figure, anything you want really, and save this figure to the `output/figures/` folder. Check your *GitHub Desktop*. What do you notice?
