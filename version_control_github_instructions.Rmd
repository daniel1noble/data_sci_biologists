...
Create: 1) a new RStudio project file, 2) download the `OA_activitydat_20190302_BIOL3207.csv` from Wattle and add this to a `data/` folder in your repo and 3) add a new Rmarkdown file in this directory that has relevant yaml header with your name, date ect. Remember how we have been setting up our RMarkdown file structure. Make sure that you mimic that structure so that you can render the html version of it.

Once added go back to your *GitHub Desktop* application. What's changed? Commit a message about why you added these files and push this up to your *GitHub* repo. Take a look at your *GitHub* repo online. What's changed?

# **Task 4**: Editing file, saving, committing and pushing to GitHub

Open up your new Rmarkdown file that you created and are now tracking in your repo. Edit this file by adding code to first load the `OA_activitydat_20190302_BIOL3207.csv`. The `OA_activitydat_20190302_BIOL3207.csv` is a data set on ocean acidification effects on fish behaviour that was published in Nature in 2020. The detailed meta-data for each column are as follows:
...
Second, there are a bunch of extra columns and missing data in the dataset. Use your data wrangling skills to explcitly code a chunk to remove the missing data, check that there are no spelling issues in variables and drop out columns that are not relevant (e.g, `comment` column and `loc` can probably go for now).
...
Not all files in a repo need to be tracked by Git. For example, anything that you create using R code in your Rmarkdown files, such as figures or rendered htmls, need not necessarily be tracked. That's because, well, the code can re-create these figures. 

In RStudio, create a text file called `.gitignore` and save this file to the root of your repo. The "root" is the main directory folder that you cloned down. 

As a first step, add `*.html` to your `.gitignore` file. Save this file. Now, knit your Rmarkdown document. What do you notice about your *GitHub Desktop* application when this file is created?

As a second demonstration, in your repo, add the following folders: `output/figures/`. In short, add a folder called `output/` in the main directory and then, **inside** the `output/` folder add a `figures/` folder. Add the path to the figures folder to your `.gitignore` file and save this file. 

In your Rmarkdown file, use the data that you stored (i.e., `OA_activitydat_20190302_BIOL3207.csv`) to create a single figure, anything you want really, and save this figure to the `output/figures/` folder. Check your *GitHub Desktop*. What do you notice?
...
:::: {.organgebox data-latex=""}
::: {.center data-latex=""}
<font size="18">**Task 6 & 7** should take ~30 minutes</font>
:::
::::
...
# **Task 7**: Pulling down changes and creating an issue on *GitHub*
...
:::: {.organgebox data-latex=""}
::: {.center data-latex=""}
<font size="18">**Task 8** should take ~20 minutes</font>
:::
::::

# **Task 8**: Resolving conflicts
