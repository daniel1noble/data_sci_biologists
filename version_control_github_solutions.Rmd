...
3) The Rmarkdown file you have created a bunch of times before in previous workshops. 
...
Right! Now that we have a bunch of files, we are ready to make changes to existing files. Just like in **Task 3** we need to keep track of the changes. To Start, we're going to edit the Rmarkdown file we created and add code chunks.


First, lets load the data file (`OA_activitydat_20190302_BIOL3207.csv`). Everyone should now have the EXACT same repo structure. As such, the code should work if they remember about how to code using relative path names. 

```{r, data, results='hide', message=FALSE}

# Load data. Again, this code should work across ALL computers.
data <- read_csv("./data/clark_et-al_2020/OA_activitydat_20190302_BIOL3207.csv")

```

The second step is to clean up the datafile to get it ready for analysis and plotting. Lots of ways to do this, but here's an example using tidyverse to remove missing data and select only relevant rows.

```{r, tidy=TRUE}
## Data wrangling; get rid of NA's and remove unnecessary 
## columns like row names from file, comments and location
data_clean <- data %>% filter(!is.na(activity)) %>% select(-c("...1", "comment", "loc"))
head(data_clean)
```

Then, we can summarises data and check for any spelling errors in a single step that uses the tidyverse:

```{r, tidy=TRUE}
## Have a look for any spelling mistakes in species / treatment 
## but also lets summarise the data: mean, se, number of fish / observations
summary_data <- data_clean %>% 
                group_by(species, treatment) %>% 
                summarise(  x = mean(activity), 
...
After you have made edits **save** your file normally. Navigate back to `GitHub Desktop`. You'll now notice that the RMarkdown file is identified as having changed. We can even see what what exactly has changed. The 'red' are the lines that were modified or removed and the `green` are the lines that were changed. 
