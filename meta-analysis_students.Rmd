...
```{r, Zrcalc}
# Calculate Fisher's r-to-z transformed correlation coefficient (in escalc it's called "ZCOR" and specified in the 'measure' argument. See ?escalc). Additional arguments that are important for this effect size include ri = raw correlation coefficients, and ni = sample size, which are needed based on the formulas above. Remember to also include the var.names = c("Zr", "V_Zr") argument.
...
+ We can also see that the null hypothesis that Zr = 0 can be rejected because there is a significantly larger estimate than a correlation of 0, which we can see from the p-value being < 0.05. To be more exact, the p-value is `r #"ADD YOUR CODE HERE"`. 
