...
samp_dist_Zr <-  #"ADD YOUR CODE HERE"
...
#### Overall meta-analytic mean
 ...
 + We can extract the estimate using the `coef` function, it is estimated to be `r #"ADD YOUR CODE HERE"`, which tells us that the mean Zr value is positive, but there is a rather weak overall association between physiology and dispersal / movement when we pool across all studies. 
...
+ Knowing this, we can extract the 95% confidence intervals which range from `r #"ADD YOUR CODE HERE"` to `r #"ADD YOUR CODE HERE"`. In other words, 95% of the time we would expect the true mean to fall between Zr values of `r #"ADD YOUR CODE HERE"` to `r #"ADD YOUR CODE HERE"`. 
...
+ We can also see that the null hypothesis that Zr = 0 can be rejected because there is a significantly larger estimate that a correlation of 0, which we can see from the p-value being < 0.05. To be more exact, the p-value is `r #"ADD YOUR CODE HERE"`. 
...
>Overall, we have highly heterogeneous effect size data because sampling variation only contributes to `r #"ADD YOUR CODE HERE"`% of the total variation in effects.  
...
>From the multilevel meta-analytic model we find that only `r #"ADD YOUR CODE HERE"`%  of the total variation in effect size estimates is the result of differences between studies.
...
>Our 95% prediction intervals are wide. Effect sizes (Zr) are expected to range from `r #"ADD YOUR CODE HERE"` to `r #"ADD YOUR CODE HERE"` 95% of the time with repeated experiments, suggesting a lot of inconsistency between studies. 
...
From the original study aim, the first thing @WuSeebacher2022 were interested in is the relationship of physiology with activity, exploration and dispersal (Zr). In addition to the random effects  --  `es_ID` (effect size identity), `study_ID`(unique study identity), `species`(unique species identity), they were also interested in comparing whether the correlation (or Zr) differences depending on whether the behavioural trait of interest was activity, exploration or dispersal. As such, we can fit a meta-regression model, (which includes fixed effects) that includes a moderator `disp_trait` (activity, exploration, or dispersal) to explicitly test whether Zr varies across these effect types.

We can easily extend our multi-level meta-analysis model, or, intercept only model, to include fixed effects (moderators). That can be done by adjusting the formula to include the fixed effect of interest:
...
Wow, there are some big changes here! Why? Well, because we are now estimating the mean and the null hypotheses being tested are different. 
