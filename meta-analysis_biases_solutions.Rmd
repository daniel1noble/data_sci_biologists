...
It turns out we can `trick` the model into estimating an intercept -- the overall meta-analytic mean -- that is more relevant and interpretable for us. To do that, we can 'centre' the `Year` variable. Remember back to *workshop 8 on Statistical Models* where we subtracted the mouse mass from the cage mean? Yep, it's the same thing! 

What does centering do for us here? Well, for simplicity, we can center on the mean of the `Year` column by simply subtracting every value of `Year` by the "mean year". This changes the variable so that 0 is now the mean Year in the data set. Why don't you do that in the code block below. Use the `tidyverse` :
...
What's really cool here is that, now, we have been able to deal with both types of publication bias in a single model and re-estimate (i.e., predict) what the mean Zr value is when sampling variance is 0 and for an average year in the dataset.

<br>

#### **Question 5: What is the average correlation between metabolism and fitness now predicted to be when we account for publication biases?**  {.tabset .tabset-fade .tabset-pills} 
