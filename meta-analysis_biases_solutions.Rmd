...
To better understand this, lets think about flipping around the funnel plot, but this time lets use the sampling variance, not precision:
...
>Now that you have completed the Rmarkdown document try rendering it as an HTML. Does it work? If not, why not? Remember that you were giving a ref.bib file. Make sure you have this file in your working directory. Also check that the yaml header (right at the top of the RMarkdown) has the correct path. If it's working one code chunk at a time try to clear your working directory (i.e., `rm(list = ls()`) and re-run each code chunk. 
